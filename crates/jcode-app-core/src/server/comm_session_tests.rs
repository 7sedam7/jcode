...
#[path = "comm_session_tests/spawn_selection.rs"]
mod spawn_selection;
...
#[tokio::test]
async fn busy_coordinator_spawn_uses_its_live_provider_not_the_server_template() {
    let coordinator = test_agent_with_working_dir("live_provider_coord", "/tmp/coord").await;
    let template: Arc<dyn Provider> = Arc::new(MockProvider {
        name: "template",
        models: vec!["stale-model".to_string()],
    });
    let _held = coordinator.lock().await;

    let resolved = crate::server::spawn_provider::for_session("live_provider_coord", &template);
