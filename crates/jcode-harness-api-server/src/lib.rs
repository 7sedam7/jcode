...
pub async fn run_bridge(api_socket: PathBuf, legacy_socket: PathBuf) -> Result<()> {
    ...
    if let Some(parent) = api_socket.parent() {
        ...
        #[cfg(unix)]
        {
            ...
            std::fs::set_permissions(parent, std::fs::Permissions::from_mode(0o700))
                .with_context(|| format!("restrict API socket directory {}", parent.display()))?;
        }
    }
    ...
    #[cfg(unix)]
    {
        ...
        let restricted_socket =
            std::fs::set_permissions(&api_socket, std::fs::Permissions::from_mode(0o600)).is_ok()
                && std::fs::metadata(&api_socket)
                    .map(|meta| meta.permissions().mode() & 0o777 == 0o600)
                    .unwrap_or(false);
        let restricted_parent = api_socket.parent().is_some_and(|parent| {
            std::fs::metadata(parent)
                .map(|meta| meta.permissions().mode() & 0o077 == 0)
                .unwrap_or(false)
        });
        anyhow::ensure!(
            restricted_socket || restricted_parent,
            "could not restrict API socket or its parent directory"
        );
    }
    eprintln!(
    ...
    async fn the_api_socket_is_owner_only() {
        ...
        for _ in 0..100 {
            if let Ok(meta) = std::fs::metadata(&api_socket) {
                ...
                let parent_owner_only = std::fs::metadata(&dir)
                    .map(|meta| meta.permissions().mode() & 0o077 == 0)
                    .unwrap_or(false);
                if observed == 0o600 || parent_owner_only {
                    mode = Some(observed);
                    break;
                }
            }
        ...
        assert!(
            mode.is_some(),
            "API socket must be 0600 or live inside an owner-only directory; a wider \
             reachable path exposes every session behind the bridge to other local users"
        );
    }
}
