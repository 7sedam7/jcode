...
pub(super) fn install_conversion_id_is_fresh(modified: SystemTime) -> bool {
    modified
        .elapsed()
        .map(|age| age <= Duration::from_secs(90 * 24 * 60 * 60))
        // Filesystem clocks can be a fraction ahead of SystemTime::now(). A
        // future timestamp cannot be stale, so keep rather than delete it.
        .unwrap_or(true)
}
